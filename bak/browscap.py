...
BROWSCAP_ROOT = getattr(settings, "BROWSCAP_ROOT")

__all__ = ["BrowserCapabilities"]
...
class BrowserCapabilities(object):
    ...
    def parse(self):
        ...
        read_ok = cfg.read([os.path.join(BROWSCAP_ROOT, name) for name in files])
        if len(read_ok) == 0:
...
if __name__ == "__main__":
    import sys, os
    bc_filename = os.path.join(BROWSCAP_ROOT, "browscap.ini")
